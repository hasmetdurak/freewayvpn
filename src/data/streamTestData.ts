  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const baseResults: TestResult[] = [
        ...
        {
          provider: 'PureVPN',
          speed: Math.floor(Math.random() * 45) + 140, // 140-185 Mbps
          ping: Math.floor(Math.random() * 22) + 18, // 18-40 ms
          reliability: Math.floor(Math.random() * 10) + 89, // 89-99%
          works: true,
          badge: '🎬 STREAMING KING',
          affiliateUrl: 'https://billing.purevpn.com/aff.php?aff=49387047',
          features: ['10 devices', 'Streaming optimized', 'Global servers'],
          discount: 82
        },
        {
          provider: 'VPN.ac',
          ...
          features: ['Military-grade security', 'Port forwarding', 'No logs'],
          discount: 75
        },
        {
          provider: 'Hide.me',
          speed: Math.floor(Math.random() * 40) + 110, // 110-150 Mbps
          ping: Math.floor(Math.random() * 28) + 22, // 22-50 ms
          reliability: Math.floor(Math.random() * 12) + 87, // 87-99%
          works: true,
          badge: '🆓 FREE PLAN',
